...
void FP_Tree_handler::initialize_transactions(string filename) {
    ...
    int num_sup_2 = (int) (num_transactions * 0.01 * support_percent);
    ...
    //considers only the elements with freq greater than support frequency
}
...
void FP_Tree_handler::condition_on(node *item_start, vector<int> previous_items, vector<vector<int>> &patterns, int &current_checks, int &max_checks){
    ...
    current_checks ++;
    if(current_checks % 100000 == 0)
    {
        cout<<"CHECK : "<<current_checks<<" | PATTERNS SIZE : "<<patterns.size()<<"\n";
    }
    ...
    if(num_paths > 1) 
    {
        patterns.push_back(previous_items);
        ...
        if(previous_items.size() <= 2)
        {
            ...
            {
                condition_on(header_list[upper_item], previous_items, patterns, current_checks, max_checks);
            }
        }
    }
    else if(num_paths == 1) 
    {
        ...
        if(if_single) 
        {
            print_all_patterns(if_single, previous_items, patterns);
        }
    }
...
// Right now this uses a set of vector<int> to keep track of different subsets
void FP_Tree_handler::print_all_patterns(node *lower_end, vector<int> previous_patterns, vector<vector<int>> &patterns) {
...
vector<vector<int>> *FP_Tree_handler::run_fp_tree(int max_checks) {
    vector<int> empty;
    vector<vector<int>> *patterns = new vector<vector<int>>();
    // int max_checks = 1e8;
    int current_checks = 0;
    for(int j = header_list.size() -1 ; j >= 0 ; --j)
        {
            // Just an empty pattern to be passed initially
            condition_on(header_list[j], empty, *patterns, current_checks, max_checks);
        }
